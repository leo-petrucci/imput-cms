...
export const select = (
  ...
  selector.addEventListener("mousedown", (e) => {
    ...
    selectorList.style.left = `${
      wrapperRect.left -
      (view.dom.parentElement?.getBoundingClientRect().left || 0)
    }px`;
    const border = utils.themeManager.get(ThemeSize, "lineWidth");
    selectorList.style.top = `calc(${
  ...
  selectorValue.innerHTML = config.text;

  const _div = document.createElement("div");
  _div.innerHTML = `<i class="ph-caret-down"></i>`;
  const selectorButton = _div;
  ...
  config.options.forEach((option) => {
    ...
    selectorListItem.innerHTML = option.text;
      ...
      return css`
        ...
        .menu-selector-list {
          ...
          ${themeManager.get(ThemeShadow, undefined)}
                    border-bottom-left-radius: ${themeManager.get(
            ThemeSize,
            "radius"
          )};
